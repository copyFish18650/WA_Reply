...
风格学习会提取最近最多 300 条去重的本人历史回复，统计整批样本的语言、句长、问候、分行、问句和 emoji 使用习惯，再均匀选取最多 24 条代表性摘录供本地模型分析，摘录总长度最多 2400 字符。风格学习默认等待最多 10 分钟，可通过 `LOCAL_AI_STYLE_TIMEOUT_MS` 调整（1000–1800000 毫秒）。
